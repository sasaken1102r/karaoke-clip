...
- **末尾会話の自動トリム**: `python scripts/trim_trailing.py <wav16k> <segments.json> --lyrics <lyrics_dir>` で全区間の「伴奏終止後の会話」を一括検出→`_trimmed.json`。★レベルだけでは無伴奏アウトロも会話と誤検出しうるので、出力の「歌詞末尾」を見て**会話(お疲れ様/雑談/ご視聴/幻覚等)ならトリム・歌詞のままなら残す**を判断してから採用。採用後 process.py に `_trimmed.json` を渡す。
